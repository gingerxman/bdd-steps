...
def step_impl(context, user, name):
	...
	resp = context.client.delete("ginger-product:product.category", {"id": id})
	bdd_util.assert_api_call_success(resp)
...
def step_impl(context, user, name):
	...
	resp = context.client.post("ginger-product:product.category", params)
	bdd_util.assert_api_call_success(resp)

@When(u"{user}启用商品分类'{name}'")
def step_impl(context, user, name):
	id = get_product_category_id_by_name(name)
	resp = context.client.delete("ginger-product:product.disabled_category", {"id": id})
	bdd_util.assert_api_call_success(resp)

@When(u"{user}禁用商品分类'{name}'")
def step_impl(context, user, name):
	id = get_product_category_id_by_name(name)
	resp = context.client.put("ginger-product:product.disabled_category", {"id": id})
	bdd_util.assert_api_call_success(resp)
...
def step_impl(context, user, name):
	...
	resp = context.client.post("ginger-product:product.category_display_index", {"id": id, "action": action})
	bdd_util.assert_api_call_success(resp)
