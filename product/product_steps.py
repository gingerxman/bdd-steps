...
def __format_product_post_data(context, product):
	...
	base_info = {
		...
		'liny_price': int(round(product.get('liny_price', 0) * 100)),
...
def __create_product(context, product):
	...
	response = context.client.put('ginger-product:product.product', data)
...
def __get_product(context, corpuser_name, name):
	...
	response = context.client.get('ginger-product:product.product', data)
	bdd_util.assert_api_call_success(response)
	...
	product = {
		...
		"shelf_status": u"上架" if base_info['shelf_status'] == 'on_shelf' else u"下架"
	}
...
def step_impl(context, user, name):
	...
	resp = context.client.delete("ginger-product:product.product", {"id": id})
...
def step_impl(context, user, name):
	id = get_product_id_by_name(name)
	resp = context.client.delete("ginger-product:product.disabled_product", {"id": id})
	bdd_util.assert_api_call_success(resp)

@When(u"{user}禁用商品'{name}'")
def step_impl(context, user, name):
	id = get_product_id_by_name(name)
	resp = context.client.put("ginger-product:product.disabled_product", {"id": id})
	bdd_util.assert_api_call_success(resp)
...
def step_impl(context, user, name):
	...
	resp = context.client.post("ginger-product:product.product_index", {"id": id, "action": action})
	bdd_util.assert_api_call_success(resp)
...
def step_impl(context, user, shelf_name):
	...
	if shelf_name == u'在售':
		data = {
			'pool_product_ids': json.dumps(pool_product_ids)
		}
		response = context.client.put('ginger-product:product.onshelf_products', data)
		bdd_util.assert_api_call_success(response)
	elif shelf_name == u'待售':
		data = {
			'pool_product_ids': json.dumps(pool_product_ids)
		}
		response = context.client.put('ginger-product:product.offshelf_products', data)
		bdd_util.assert_api_call_success(response)
